...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None,folder=None):
    ...
    # Fix: Update init AFTER extracting and modifying it
    init = list(info['init'])
    init[0] = plans_file  # Replace hardcoded path with local path
    ...
    assert issubclass(tr, nnFormerTrainer), "The network trainer was found but is not a subclass of nnFormerTrainer.Please make it so!"
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best"):
    ...
    # weights_only=False needed for older checkpoints with numpy arrays (PyTorch 2.6+ security change)
